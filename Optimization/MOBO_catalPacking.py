...
def generate_initial_data():
    ...
    expData_x = pd.read_csv('data_x.csv')
    expData_y = pd.read_csv('data_y.csv')
    train_x = torch.tensor(expData_x.values[:,0:2]).to(**tkwargs)
    train_obj = torch.tensor(expData_y.values[:,0:2]).to(**tkwargs)
...
x_all.columns=['x1','x2']
x_all.to_csv('data_x.csv',index=False)
print(volume)
